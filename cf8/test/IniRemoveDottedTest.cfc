<cfcomponent extends="mxunit.framework.TestCase">
	...
	<cffunction name="beforeTests">
		...
		ini = createObject("component", "ini.cf8.Ini").init(filePath, true);
		</cfscript>
	</cffunction>
	...
	<cffunction name="testInheritedSectionHasCorrectKeyCount">
		...
		assertEquals(structCount(section), 10);
		</cfscript>
	</cffunction>
	...
	<cffunction name="testInheritedSectionHasCorrectKeys">
		...
		assertFalse(StructKeyExists(section, "nested.foo"));
		assertFalse(StructKeyExists(section, "nested.bar"));
		assertFalse(StructKeyExists(section, "nested.baz"));
	...
	<cffunction name="testDottedPropertyDoesNotExists">
		<cfscript>
		var section = ini.getSection("development");
